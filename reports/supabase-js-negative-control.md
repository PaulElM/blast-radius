...
- **⚠️ Wildcard entry points cannot be enumerated, and this package declares 1: `./dist/*`.** A wildcard subpath is absent from the 3 entry points this report diffed, so `@supabase/supabase-js` exposes reachable import paths that were **never compared between the two versions**. Consumers in this corpus import **1** subpath that the manifest does not declare; each was resolved against both installed tarballs, and **0** of them resolve in `2.110.8` and no longer resolve in `3.0.0-next.29`. What remains uncounted is any wildcard subpath **no scanned consumer imports** — that part is still not quantifiable from the manifest, and it pushes the real figure up, never down.
